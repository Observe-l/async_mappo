...
This demo visualizes a trained async-MAPPO policy (trained on the offline `rul_schedule` environment) acting in a live SUMO simulation. The policy uses the offline environment for decisions; SUMO is used to route and animate trucks between ParkingAreas `Factory0..Factory49`.
...
```bash
python scripts/render/render_sumo_schedule.py \
  --actor_dir /home/lwh/Documents/Code/results/async_schedule/rul_schedule/mappo/threshold_7/wandb/run-20250503_002045-r5psc472/file \
  --sumo_cfg /home/lwh/Documents/Code/RL-Scheduling/map/sg_map/osm.sumocfg \
  --num_agents 4 \
  --episodes 1 \
  --max_steps 20 \
...
- `--max_steps`: Max offline-env decision steps per episode (termination condition).
