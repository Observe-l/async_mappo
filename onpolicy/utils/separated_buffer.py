...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space):
        ...
        self.asynch = getattr(args, "asynch", False)

        self._mixed_obs = False  # dict observation support

        obs_shape = get_shape_from_obs_space(obs_space)
        share_obs_shape = get_shape_from_obs_space(share_obs_space)

        # for mixed observation (gym.spaces.Dict)
        if 'dict' in obs_shape.__class__.__name__:
            ...
            for key in obs_shape:
                self.obs[key] = np.zeros(
                    (self.episode_length + 1, self.n_rollout_threads, *obs_shape[key].shape),
                    dtype=np.float32,
                )
            for key in share_obs_shape:
                self.share_obs[key] = np.zeros(
                    (self.episode_length + 1, self.n_rollout_threads, *share_obs_shape[key].shape),
                    dtype=np.float32,
                )
        else:
        ...
        # for async schedule: last inserted step per env thread
        self.update_step = np.zeros((self.n_rollout_threads, 1), dtype=np.int32)
    ...
    def async_insert(
        ...
        """Async insertion for per-agent buffer.

        active_envs: list of tuples (env_id, step_cnt) OR (env_id, agent_id, step_cnt).
        step_cnt follows AsyncControl.cnt semantics (starts at 0, then increments before insertion).
        """
        assert active_envs is not None
        for item in active_envs:
            if len(item) == 2:
                e, step_cnt = item
            else:
                e, _, step_cnt = item
            ...
            step = int(step_cnt - 1)
            if step == -1:
                step = int(self.update_step[e, 0])
                if self._mixed_obs:
                    for key in self.share_obs.keys():
                        self.share_obs[key][step + 1, e] = share_obs[key][e].copy()
                    for key in self.obs.keys():
                        self.obs[key][step + 1, e] = obs[key][e].copy()
                else:
                    self.share_obs[step + 1, e] = share_obs[e].copy()
                    self.obs[step + 1, e] = obs[e].copy()
                self.rnn_states[step + 1, e] = rnn_states[e].copy()
                self.rnn_states_critic[step + 1, e] = rnn_states_critic[e].copy()
                self.masks[step + 1, e] = masks[e].copy()
                if bad_masks is not None:
                    self.bad_masks[step + 1, e] = bad_masks[e].copy()
                if active_masks is not None:
                    self.active_masks[step + 1, e] = active_masks[e].copy()
                if available_actions is not None:
                    self.available_actions[step + 1, e] = available_actions[e].copy()
                continue
            ...
            self.rnn_states[step + 1, e] = rnn_states[e].copy()
            self.rnn_states_critic[step + 1, e] = rnn_states_critic[e].copy()
            self.actions[step, e] = actions[e].copy()
            self.action_log_probs[step, e] = action_log_probs[e].copy()
            self.value_preds[step, e] = value_preds[e].copy()
            self.rewards[step, e] = rewards[e].copy()
            self.masks[step + 1, e] = masks[e].copy()
            if bad_masks is not None:
                self.bad_masks[step + 1, e] = bad_masks[e].copy()
            if active_masks is not None:
            ...
            self.update_step[e, 0] = step

    def update_mask(self, steps):
        """Mask out timesteps after each env's last step.

        steps: array-like (n_rollout_threads,) with per-env executed step counts.
        """
        self.active_masks = np.ones_like(self.active_masks)
        for e in range(self.n_rollout_threads):
            # AsyncControl.cnt is a count; last filled transition index is (cnt-1).
            last_step = max(int(steps[e]) - 1, 0)
            self.masks[last_step + 1 :, e] = np.zeros_like(self.masks[last_step + 1 :, e])
            self.active_masks[last_step + 1 :, e] = np.zeros_like(self.active_masks[last_step + 1 :, e])
    ...
    def after_update(self):
        if self.asynch:
            for e in range(self.n_rollout_threads):
                step = int(self.update_step[e, 0]) + 1
                if self._mixed_obs:
                    for key in self.share_obs.keys():
                        self.share_obs[key][0, e] = self.share_obs[key][step, e].copy()
                    for key in self.obs.keys():
                        self.obs[key][0, e] = self.obs[key][step, e].copy()
                else:
                    self.share_obs[0, e] = self.share_obs[step, e].copy()
                    self.obs[0, e] = self.obs[step, e].copy()
                self.rnn_states[0, e] = self.rnn_states[step, e].copy()
                self.rnn_states_critic[0, e] = self.rnn_states_critic[step, e].copy()
                self.masks[0, e] = self.masks[step, e].copy()
                self.bad_masks[0, e] = self.bad_masks[step, e].copy()
                self.active_masks[0, e] = self.active_masks[step, e].copy()
                if self.available_actions is not None:
                    self.available_actions[0, e] = self.available_actions[step, e].copy()
        else:
            if self._mixed_obs:
                for key in self.share_obs.keys():
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        for indices in sampler:
            ...
            if self._mixed_obs:
                share_obs_batch = {k: v[indices] for k, v in share_obs.items()}
                obs_batch = {k: v[indices] for k, v in obs.items()}
            else:
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        if self._mixed_obs:
            ...
            obs = {}
            for key in self.share_obs.keys():
                arr = self.share_obs[key]
                if len(arr.shape) > 3:
                    # (T+1, N, ...) -> (N, T, ...) -> flatten to (T*N, ...)
                    share_obs[key] = arr[:-1].transpose(1, 0, *range(2, len(arr.shape))).reshape(-1, *arr.shape[2:])
                else:
                    share_obs[key] = _cast(arr[:-1])
            for key in self.obs.keys():
                arr = self.obs[key]
                if len(arr.shape) > 3:
                    obs[key] = arr[:-1].transpose(1, 0, *range(2, len(arr.shape))).reshape(-1, *arr.shape[2:])
                else:
                    obs[key] = _cast(arr[:-1])
        else:
            if len(self.share_obs.shape) > 3:
                share_obs = self.share_obs[:-1].transpose(1, 0, *range(2, len(self.share_obs.shape))).reshape(
                    -1, *self.share_obs.shape[2:]
                )
                obs = self.obs[:-1].transpose(1, 0, *range(2, len(self.obs.shape))).reshape(-1, *self.obs.shape[2:])
            else:
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                if self._mixed_obs:
                    share_obs_batch.append({k: v[ind:ind + data_chunk_length] for k, v in share_obs.items()})
                    obs_batch.append({k: v[ind:ind + data_chunk_length] for k, v in obs.items()})
                else:
            ...
            if self._mixed_obs:
                ...
                for key in share_obs.keys():
                    share_obs_stacked[key] = np.stack([b[key] for b in share_obs_batch])
                for key in obs.keys():
                    obs_stacked[key] = np.stack([b[key] for b in obs_batch])
                share_obs_batch = share_obs_stacked
                obs_batch = obs_stacked
            else:
