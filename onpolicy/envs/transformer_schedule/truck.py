...
class Truck(object):
    ...
    def reset(self, weight:float = 0.0, state:str = 'waiting', product:str = 'A') -> None:
        ...
        self.eng_state = np.load(
            f"/home/lwh/Documents/Code/RL-Scheduling/util/cisco_engine_data/engine_{self.eng_id}.npz"
    ...
    def eng_add_obs(self, obs) -> None:
        ...
        '''
        tmp_obs = np.asarray(obs, dtype=np.float32)
        # 训练脚本输入是 (T, 18) 的序列，这里每一步统一保存为 (18,) 的向量。
        if tmp_obs.ndim == 2 and tmp_obs.shape[0] == 1:
            tmp_obs = tmp_obs[0]
        if tmp_obs.ndim != 1:
            raise ValueError(f"Engine obs must be 1D or (1,F), got shape={tmp_obs.shape}")
